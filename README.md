...
**왜 환각을 "사실 오류"로 정의하고 창작과 구분했는가**: LLM은 확률적 생성 방식(다음 토큰의 확률을 예측해 이어붙임)으로 동작하며, "이 문장이 사실인지 검증하는 단계"가 구조적으로 없습니다. 그래서 사실/수치/정책/계산처럼 **검증 가능한 정답이 실제로 존재하는 영역**에서 근거 없이 확신에 찬 답을 내놓으면, 사용자가 이를 사실로 믿고 잘못된 의사결정을 내릴 위험이 생깁니다. 반대로 창작(소설/카피)은 애초에 "검증 가능한 정답"이 존재하지 않는 영역이라 같은 잣대를 적용할 수 없습니다 — 다만 허구임을 숨기거나 스스로 세운 세계관 규칙을 어기면 그건 다시 신뢰성 문제이므로 별도 단서를 뒀습니다. 기준은 "검증 가능한 근거가 존재하는가"이며, 존재하면 근거 없는 확신은 환각이고, 존재하지 않으면(순수 창작) 환각 개념이 적용되지 않습니다.
